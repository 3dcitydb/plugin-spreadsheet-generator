...
public class SimpleQuery {
    ...
    private BoundingBox bboxFilter;
    ...
    public BoundingBox getBboxFilter() {
        return bboxFilter;
    }
    ...
    public void setBboxFilter(BoundingBox bboxFilter) {
        this.bboxFilter = bboxFilter;
    }

    public org.citydb.config.project.exporter.SimpleQuery toSimpleQuery() {
        org.citydb.config.project.exporter.SimpleQuery simpleQuery = new org.citydb.config.project.exporter.SimpleQuery();

        simpleQuery.setUseTypeNames(useTypeNames);
        simpleQuery.setUseFeatureVersionFilter(useFeatureVersionFilter);
        simpleQuery.setUseAttributeFilter(useAttributeFilter);
        simpleQuery.setUseSQLFilter(useSQLFilter);
        simpleQuery.setUseBboxFilter(useBboxFilter);

        simpleQuery.setFeatureTypeFilter(featureTypeFilter);
        simpleQuery.setFeatureVersionFilter(featureVersionFilter);
        simpleQuery.setAttributeFilter(attributeFilter);
        simpleQuery.setSQLFilter(sqlFilter);

        if (useBboxFilter) {
            SimpleTiling tiling = new SimpleTiling();
            tiling.setExtent(bboxFilter);
            simpleQuery.setBboxFilter(tiling);
        }
